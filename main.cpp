...
void WinHotkeyRoutine(int) {
	...
	if (RegisterHotKey(
		...
		0x57))  //0x51 is 'W'
	{
		printf("Hotkey 'ALT+W' registered, using MOD_NOREPEAT flag\n");
	}
	...
	if (RegisterHotKey(
		...
		0x45))  //0x51 is 'E'
	{
		printf("Hotkey 'ALT+E' registered, using MOD_NOREPEAT flag\n");
	}

	MSG msg = { 0 };
	BOOL bRet;
	while (true) {
		while ((bRet = PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) != 0) {
			if (bRet == -1) {
				...
				if (msg.message == WM_HOTKEY) {
					switch (msg.wParam) {
					case KEYID_Q:
						printf("Hotkey 'ALT+Q' received\n");
						break;
					case KEYID_W:
						printf("Hotkey 'ALT+W' received\n");
						break;
					case KEYID_E:
						printf("Hotkey 'ALT+E' received\n");
						break;
					}
				}
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, INT iCmdShow) {
	std::thread hotkeyTread(&WinHotkeyRoutine, 0);

	while (true) {
	}

	PostThreadMessage((DWORD)hotkeyTread.native_handle(), WM_QUIT, 0, 0);
